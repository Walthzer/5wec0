 /*
 *  TU/e 5WEC0::PYNQ RYB Communication Demonstration. Crying module
 *
 *  Interface:
...
int main(void) {
  ...
  ui_rprintf(&ui, 1, "%q%w%tCrying", RGB_WHITE, RGB_ORANGE, COMIC);
  //interface layout
  //Outputs: Crying
  ...
  uint32_t values[10] = {-10,2,7,19,45,50,72,93,120};
  while (get_switch_state(1))
  {
    ...
    uint32_t crying = value;

    //outputs
    com_put(&com, CRYING, crying);
    com_get(&com, CRYING, &crying);
    
    ui_rprintf(&ui, 4, "%qValue: %q%d", RGB_PURPLE, RGB_GREEN, value);
    ui_rprintf(&ui, 5, "%qCrying[0-100]: %q%d", RGB_PURPLE, RGB_GREEN, crying);
